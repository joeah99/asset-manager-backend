    ...
    public class ValuationDbContext
    {
        ...
        public async Task<List<EquipmentValuationDTO>> GetEquipmentValuationsAsync(int user_id)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var command = new MySqlCommand(
                    ...
                    FROM dbo.EquipmentValuationLog
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
                        var equipmentValuation = new EquipmentValuationDTO
                        {
                            ...
                            UnadjustedFairMarketValue = reader.IsDBNull(reader.GetOrdinal("unadjusted_fair_market_value"))
                                                    ? 0 : reader.GetDouble(reader.GetOrdinal("unadjusted_fair_market_value")),
                            UnadjustedOrderlyLiquidationValue = reader.IsDBNull(reader.GetOrdinal("unadjusted_orderly_liquidation_value"))
                                                            ? 0 : reader.GetDouble(reader.GetOrdinal("unadjusted_orderly_liquidation_value")),
                            UnadjustedForcedLiquidationValue = reader.IsDBNull(reader.GetOrdinal("unadjusted_forced_liquidation_value"))
                                                        ? 0 : reader.GetDouble(reader.GetOrdinal("unadjusted_forced_liquidation_value")),
                            AdjustedFairMarketValue = reader.IsDBNull(reader.GetOrdinal("adjusted_fair_market_value"))
                                                ? 0 : reader.GetDouble(reader.GetOrdinal("adjusted_fair_market_value")),
                            AdjustedOrderlyLiquidationValue = reader.IsDBNull(reader.GetOrdinal("adjusted_orderly_liquidation_value"))
                                                        ? 0 : reader.GetDouble(reader.GetOrdinal("adjusted_orderly_liquidation_value")),
                            AdjustedForcedLiquidationValue = reader.IsDBNull(reader.GetOrdinal("adjusted_forced_liquidation_value"))
                                                        ? 0 : reader.GetDouble(reader.GetOrdinal("adjusted_forced_liquidation_value")),
                            Salvage = reader.IsDBNull(reader.GetOrdinal("salvage"))
                                ? 0 : reader.GetDouble(reader.GetOrdinal("salvage"))
                        };
        ...
        public async Task InsertEquipmentValuationAsync(EquipmentValuationDTO valuation, long assetId)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var insertCommand = new MySqlCommand(
                    @"INSERT INTO dbo.EquipmentValuationLog (asset_id, unadjusted_fair_market_value, 
                      ...
                      VALUES (@asset_id, @unadjusted_fair_market_value, @unadjusted_orderly_liquidation_value, 
                ...
                insertCommand.Parameters.AddWithValue("@valuation_date", DateTime.Now);
        ...
        public async Task InsertVehicleValuationAsync(VehicleValuationDTO valuation, long assetId)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var insertCommand = new MySqlCommand(
                    @"INSERT INTO dbo.VehicleValuationLog (asset_id, unadjusted_low, unadjusted_high, 
                      unadjusted_finance, unadjusted_retail, unadjusted_wholesale, unadjusted_trade_in, adjusted_low, 
                      adjusted_high, adjusted_finance, adjusted_retail, adjusted_wholesale, adjusted_trade_in, 
                      valuation_date)
                      VALUES (@asset_id, @unadjusted_low, @unadjusted_high, @unadjusted_finance, 
                      @unadjusted_retail, @unadjusted_wholesale, @unadjusted_trade_in, @adjusted_low, @adjusted_high, 
                      @adjusted_finance, @adjusted_retail, @adjusted_wholesale, @adjusted_trade_in, @valuation_date);", conn);
        ...
        public async Task<List<AdjustedForcedLiquidation>> GetAdjustedForcedLiquidationAsync(int user_id)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var command = new MySqlCommand(
                  ...
                  asset_id, 
                  valuation_date, 
                  adjusted_forced_liquidation_value
              FROM dbo.EquipmentValuationLog
              WHERE asset_id IN (SELECT asset_id FROM dbo.Asset WHERE user_id = @user_id)", conn);
                command.Parameters.AddWithValue("@user_id", user_id);
