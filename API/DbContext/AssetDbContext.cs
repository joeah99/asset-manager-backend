...
namespace API.DbContext
{
      ...
      using (var conn = new MySqlConnection(_connectionString))
      {
        ...
        var command = new MySqlCommand(
            @"SELECT 
                      ...
                      FROM dbo.Asset
                      WHERE user_id = @user_id AND deleted <> 1", conn);
  ...
  public async Task<List<AssetDTO>> GetAllAssetsAsync()
    ...
    public async Task<AssetDTO?> CreateAssetAsync(AssetDTO asset)
    {
      using (var conn = new MySqlConnection(_connectionString))
      {
        ...
        var insertCommand = new MySqlCommand(
            @"INSERT INTO dbo.Asset (user_id, asset_type, initial_book_value, manufacturer, model, model_year, usage, condition, country, state_us, depreciation_method,
              salvage_value, useful_life, depreciation_rate, total_expected_units_production, units_produced_in_year, created_at, updated_at)
              VALUES (@user_id, @asset_type, @initial_book_value, @manufacturer, @model, @model_year, @usage, @condition, @country, @state_us, @depreciation_method,
        ...
        var newAssetId = (decimal)await insertCommand.ExecuteScalarAsync();
        asset.AssetId = (long)newAssetId;
    ...
    public async Task DeleteAssetAsync(AssetDTO asset)
    {
      try
      {
        using (var conn = new MySqlConnection(_connectionString))
        {
            ...
            var deleteCommand = new MySqlCommand(@"UPDATE dbo.Asset SET deleted = 1 WHERE user_id = @user_id AND asset_id = @asset_id", conn);
    ...
    public async Task<AssetDTO> UpdateAssetAsync(AssetDTO asset)
    {
      ...
      {
        throw new Exception($"Error updating asset {asset.AssetId}: {e.Message}");
      }
    }
